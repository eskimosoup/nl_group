...
RSpec.describe TeamMemberPresenter, type: :presenter do
  ...
  describe "with image" do
    ...
    it "should return image with click through" do
      expect(team_member_presenter.profile_preview).to eq(link_to team_member_presenter.small_image(alt: team_member_presenter.name), team_member, class: 'colorbox css-animated-hover', rel: 'gallery', data: { class: 'tada' })
    end
    it "should return image with click through" do
  ...
  describe "homepage classes" do
    ...
    it "should always return large-2 columns" do
      expect(team_member_presenter.homepage_classes).to eq("large-2 columns")
    end

    it "should be offset 2 if the outer loop index is 5" do
      expect(team_member_presenter.homepage_classes(outer_loop: 5)).to eq("large-2 columns large-offset-6")
    end

    it "should offset 2 if the outer loop index is 0 and the inner loop is 1" do
      expect(team_member_presenter.homepage_classes(outer_loop: 0, inner_loop: 1)).to eq("large-2 columns large-offset-2")
    end

    it "should offset 2 and end if the outer loop index is 1 and the inner loop is 1" do
      expect(team_member_presenter.homepage_classes(outer_loop: 1, inner_loop: 1)).to eq("large-2 columns large-offset-2 end")
    end

    it "should offset 4 and end if the outer loop index is 4 and the inner loop is 2" do
      expect(team_member_presenter.homepage_classes(outer_loop: 4, inner_loop: 2)).to eq("large-2 columns large-offset-4 end")
    end

    it "should offset 2 if the outer loop index is 3 and the inner loop is 1" do
      expect(team_member_presenter.homepage_classes(outer_loop: 3, inner_loop: 1)).to eq("large-2 columns large-offset-2")
    end

    it "should end if the outer loop index is 3 and the inner loop is 3" do
      expect(team_member_presenter.homepage_classes(outer_loop: 3, inner_loop: 3)).to eq("large-2 columns end")
    end

    it "should offset 2 if the outer loop index is 2 and the inner loop is 0" do
      expect(team_member_presenter.homepage_classes(outer_loop: 2, inner_loop: 0)).to eq("large-2 columns large-offset-2")
    end

    it "should offset 2 end if the outer loop index is 2 and the inner loop is 2" do
      expect(team_member_presenter.homepage_classes(outer_loop: 2, inner_loop: 2)).to eq("large-2 columns large-offset-2 end")
    end

    it "should end if the outer loop index is 2 and the inner loop is 3" do
      expect(team_member_presenter.homepage_classes(outer_loop: 2, inner_loop: 3)).to eq("large-2 columns end")
    end
  end
end
